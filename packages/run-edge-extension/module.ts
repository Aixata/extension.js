...
import RunEdgePlugin from './steps/RunEdgePlugin'

export default class RunEdgeExtension {
  ...
  /**
   * RunEdgeExtension works by creating a WebSockets server
  ...
  apply(compiler: Compiler) {
    ...
    // for everything else, for now opening the edge://extension page on startup.
    // It starts a new browser instance with the user extension loaded.
    new RunEdgePlugin(this.options).apply(compiler)
  }
}
